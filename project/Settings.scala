...
object Settings {
  ...
  private[this] def commonSettings(projectName: String): Def.SettingsDefinition = Seq(
    ...
    Test / envVars := Map(
      ...
      "JDBC_CONNECT_URL_CATALOGUE" -> "jdbc:postgresql://localhost:5432/test_db_catalogue",
      "JDBC_USER_CATALOGUE" -> "test_jdbc_user_catalogue",
      "JDBC_PASSWORD_CATALOGUE" -> "test_jdbc_password_catalogue",
      "JDBC_CONNECT_URL_LENDING" -> "jdbc:postgresql://localhost:5432/test_db_lending",
      "JDBC_USER_LENDING" -> "test_jdbc_user_lending",
      "JDBC_PASSWORD_LENDING" -> "test_jdbc_password_lending",
      "KAFKA_BOOTSTRAP_SERVERS" -> "localhost:29092",
      "KAFKA_CONSUMER_GROUP_CATALOGUE" -> "test_kafka_consumer_group_catalogue",
      "KAFKA_CONSUMER_GROUP_LENDING" -> "test_kafka_consumer_group_lending",
      "KAFKA_SCHEMA_REGISTRY" -> "http://localhost:8081",
      "KAFKA_TOPIC_BOOK_INSTANCES" -> "test_kafka_topic_book_instances",
      "KAFKA_TOPIC_BOOK_STATE_CHANGES" -> "test_kafka_topic_book_state_changes",
      "KAFKA_TOPIC_BOOK_STATE_ERRORS" -> "test_kafka_topic_book_state_errors",
    ),
    scalacOptions ++= Seq(
      "-Xfatal-warnings",
      "-deprecation",
